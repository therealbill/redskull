...
// DialWithConfig provides a means to specify a DialConfig for greater control
// over your connection.
func DialWithConfig(cfg *DialConfig) (*Redis, error) {
	if cfg == nil {
		cfg = &DialConfig{}
	}
	if cfg.Network == "" {
...
func DialURL(rawurl string) (*Redis, error) {
	...
	return DialWithConfig(&DialConfig{ul.Scheme, ul.Host, db, password, timeout, maxidle})
}
