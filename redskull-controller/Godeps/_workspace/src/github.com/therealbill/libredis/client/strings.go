...
// SetMX sets key to hold the string value, andn only sets if the key already exists.
// Any previous time to live associated with the key is discarded on successful SET operation.
func (r *Redis) SetMX(key, value string) error {
	rp, err := r.ExecuteCommand("SET", key, value, "XX")
	if err != nil {
		return err
	}
	return rp.OKValue()
